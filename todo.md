...
- **이미지 포함 DOCX 등록 시 Vercel에서만 "저장에 실패했습니다"/"미리보기 변환 중 오류가 발생했습니다"**: 로컬 개발 서버에서는 재현 안 됐고 배포된 사이트에서만 실패. 1차로 `maxDuration = 60`을 관련 라우트에 추가했으나(타임아웃 대비), 사용자가 실제로 5MB 미만의 이미지 포함 문서로 재현 계속 — 진짜 원인은 **Vercel Serverless Function의 4.5MB 요청 본문 크기 제한**(공식 문서로 확인: [vercel.com/docs/functions/limitations](https://vercel.com/docs/functions/limitations))이었음. `maxDuration`으로는 절대 못 고치는 별개 제약 — 이미지가 포함된 DOCX(20MB까지 허용한다고 안내 중)나 5MB 표지 이미지가 요청 본문에 실리는 순간 Vercel이 함수 진입 전에 413로 거부하고, 클라이언트는 정상 JSON 대신 Vercel 자체 에러 페이지를 받아 파싱 실패 → 진단 정보 없는 뭉뚱그린 에러 메시지만 노출됨.
  - **수정**: Vercel 공식 가이드가 권장하는 "브라우저에서 스토리지로 직접 업로드" 패턴을 Supabase Storage의 signed upload URL(`createSignedUploadUrl`/`uploadToSignedUrl`)로 구현 — 새 라우트 `POST /api/admin/books/upload-url`이 관리자 인증 후 짧은 JSON 응답(서명된 업로드 URL)만 돌려주고, 실제 파일 바이트는 브라우저에서 Supabase로 직접 전송되어 Vercel 함수의 요청 본문을 전혀 거치지 않음. 기존 `/api/admin/books`(POST/PATCH)·`/preview`는 이제 파일을 직접 받는 대신, 클라이언트가 먼저 Storage에 올려둔 파일을 서버가 `download()`로 다시 받아와 기존 변환 로직을 그대로 수행. 표지 이미지는 `book-originals`(DOCX 전용 MIME 제한 버킷이라 이미지 업로드 시 거부됨을 뒤늦게 발견해 수정)가 아니라 `book-covers`에 스테이징.
  - **검증**: 실제 관리자 UI로 (a) ~440KB 이미지 포함 DOCX만, (b) DOCX+표지 이미지 함께, 두 시나리오 모두 Playwright로 실제 로그인→업로드→저장까지 구동 — 미리보기에 이미지 정상 표시(`naturalWidth` 확인), 저장 성공, DB의 `content_html`/`cover_url`이 실제 로드 가능한 Storage URL을 가리키는 것까지 확인. `npm run typecheck`/`lint`/`build`/유닛 35개/통합 10개/E2E 13개 전부 통과. 테스트로 만든 도서·Storage 객체 정리 완료.
  - **한계**: Vercel의 4.5MB 제한 자체는 로컬 환경에 존재하지 않아 로컬에서 "제한을 우회했다"는 사실 자체는 재현·검증 불가 — 다만 새 아키텍처(직접 업로드 + 참조 기반 서버 처리)가 올바르게 동작하는 것은 실제 Supabase 프로젝트 대상으로 완전히 검증됨. 재배포 후 사용자가 실제 문서로 최종 확인 필요.
