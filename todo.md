...
git 저장소 생성(2026-07-17) — 이전까지는 git 저장소가 아니었음. `main` 브랜치에 초기 커밋 완료. 프로덕션 Supabase는 지금까지 개발에 써온 프로젝트(`ittjmpowdbpcrjcvbqfx`)를 그대로 사용하기로 확정(사용자 확인) — 별도 프로덕션 전용 프로젝트를 새로 만들지 않음.

- [ ] Vercel 프로젝트 생성 — GitHub 원격 저장소 생성 및 push 대기 중(사용자 액션 필요, 아래 참고)
- [ ] Supabase 프로덕션 환경 변수 설정 — 로컬 `.env.local` 값을 Vercel 프로젝트 환경 변수로 동일하게 등록 필요(Vercel 프로젝트 생성 후)
...
- [ ] 오류 로깅 설정 — Vercel 배포 후 진행(계정 필요 없는 Vercel 기본 함수 로그부터 시작, Sentry 등 외부 서비스는 필요 시 추가)
...
- [ ] 완료 기준 확인: 프로덕션 도메인에서 서비스 작동 / DOCX 등록→공유 링크 열람 전체 흐름 성공 — Vercel 배포 완료 후 진행

### 남은 절차 (사용자 액션 필요)

1. GitHub에 빈 원격 저장소 생성 (private 권장) 후 저장소 URL 전달 — 또는 `gh auth login`을 로컬에서 직접 실행해 인증 완료
2. `git remote add origin <url> && git push -u origin main`
3. Vercel 대시보드에서 "Add New Project" → 방금 push한 GitHub 저장소 Import
4. Vercel 프로젝트 환경 변수에 `.env.local`과 동일한 값 등록: `NEXT_PUBLIC_SITE_URL`(배포 도메인으로), `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`
5. 배포 후 실제 도메인에서 DOCX 등록 → 공개 전환 → 공유 링크 열람까지 전체 흐름 재검증
